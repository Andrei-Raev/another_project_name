...
class Item(Entity):
    ...
    def render(self, surf, ccc):
        ...
        pygame.draw.circle(surf, (0, 0, 0),
                           ((self.rect.x + ccc[0]), (self.rect.y + ccc[1] )), 100)
        surf.blit(self.image, self.rect)
        print(self.rect)
        # save_s(surf)


class Dress(pygame.sprite.Sprite):
...
class World:  # Класс мира
    ...
    def init(self):
        for y in range(-1, 2):
            for x in range(-1, 3):
                self.chunks.add(
                    Chunk(0, (x + self.center_chunk_cord[0], y + self.center_chunk_cord[1])))
    ...
    def render(self, surf):
        wid = 510
        tmp_world_surf = pygame.Surface((510* 4, (510) * 3))
        tmp_items = list()
        ...
        for y in range(-1 + self.center_chunk_cord[1], 3 + self.center_chunk_cord[1]):
            for x in range(-1 + self.center_chunk_cord[0], 2 + self.center_chunk_cord[0]):
                ...
                try:
                    ...
                    self.chunks.add(Chunk(0, chunk_cord))
        ...
        surf.blit(tmp_world_surf, [i - 510 for i in [0,0]])
